...
_SQL_SYSTEM = (
    ...
    "- Keep the SELECT list consistent with your aggregation: every "
    "non-aggregated column must appear in GROUP BY, and any aggregate used in "
    "HAVING or ORDER BY (e.g. SUM(x)) MUST also be in the SELECT list, aliased "
    "(e.g. SUM(x) AS total_x), so the displayed values match the filter. Never "
    "show a raw column next to an aggregate condition — that returns an "
    "arbitrary row's value in SQLite.\n"
    "- Use WHERE to filter individual rows and HAVING only for conditions on "
    "aggregates. If a column already holds the total you need, filter it "
    "directly with WHERE instead of GROUP BY + HAVING.\n"
