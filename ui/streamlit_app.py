"""Streamlit frontend for AskDB: ask a question, see SQL + table + chart.
...
st.set_page_config(page_title="AskDB", page_icon="🧮", layout="wide")
...
def _query_via_api(question: str) -> dict | None:
...
def _query_in_process(question: str) -> dict:
...
def run_query(question: str) -> dict:
    ...
    result = _query_via_api(question)
    if result is None:
        logger.info("API unreachable at %s; answering in-process.", settings.api_base)
        result = _query_in_process(question)
    return result


def _render_result(result: dict) -> None:
    """Render summary, SQL, table, and an auto-chart from a backend response."""
    if result.get("error"):
...
def main() -> None:
    ...
    for col, sample in zip(chip_cols, SAMPLE_QUESTIONS, strict=True):
    ...
    question = st.text_input(
        ...
        placeholder="e.g. What were the top 5 products by revenue in 2023?",
    )
    ask = st.button("Ask", type="primary")

    if ask and question.strip():
        with st.spinner("Thinking… writing and running SQL"):
